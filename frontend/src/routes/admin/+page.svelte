<script lang="ts">
  ...
  $: totalRevenue = bookings
    .filter(b => b.status === 'confirmed' || b.status === 'completed' || b.status === 'paid')
    .reduce((sum, b) => {
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
  <div class="mb-8">
    ...
    <p class="text-dark-400 mt-2">Manage bookings, rooms, members, and content</p>
  </div>
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
    <div class="card">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-dark-400">Today</p>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="text-2xl font-bold text-white">{pendingMembers.length}</p>
    ...
    <div class="card">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="text-2xl font-bold text-white">{openReports.length}</p>
          <p class="text-xs text-dark-400">Reports</p>
        </div>
      </div>
    </div>
  </div>
  ...
  {#if loading}
    <div class="space-y-4">
      {#each [1, 2, 3] as _}
        <div class="card animate-pulse"><div class="h-16 bg-dark-700 rounded"></div></div>
      {/each}
    </div>
        ...
        <div class="space-y-3">
          {#each bookings.slice(0, 5) as booking}
            <div class="flex items-center justify-between py-2 border-b border-dark-700 last:border-0">
              <div>
                <p class="text-sm text-white">{booking.room?.name || 'Room'} - {booking.profile?.full_name || booking.guest_name}</p>
                <p class="text-xs text-dark-400">{formatDate(booking.date)}</p>
              </div>
              <span class={getStatusBadge(booking.status)}>{booking.status}</span>
            </div>
          {/each}
          {#if bookings.length === 0}
            <p class="text-sm text-dark-400 text-center py-4">No bookings yet</p>
          {/if}
        </div>
      </div>
      ...
      <div class="space-y-6">
        <div class="card">
          ...
          {#if pendingMembers.length === 0}
            <p class="text-sm text-dark-400">No pending members</p>
          {:else}
            <div class="space-y-3">
              {#each pendingMembers.slice(0, 5) as member}
                <div class="flex items-center justify-between py-2 border-b border-dark-700 last:border-0">
                  <div>
                    <p class="text-sm text-white">{member.full_name}</p>
                    <p class="text-xs text-dark-400">{member.email}</p>
                  </div>
                  <button on:click={() => approveMember(member.id)} class="text-sm text-green-400 hover:text-green-300 px-2 py-1 rounded hover:bg-green-900/20">
                    Approve
                  </button>
                </div>
              {/each}
        ...
        <div class="card">
          <h3 class="text-lg font-semibold text-white mb-4">Open Reports</h3>
          {#if openReports.length === 0}
            <p class="text-sm text-dark-400">No open reports</p>
          {:else}
            <div class="space-y-3">
              {#each openReports.slice(0, 5) as report}
                <div class="flex items-center justify-between py-2 border-b border-dark-700 last:border-0">
                  <div>
                    <p class="text-sm text-white">{report.subject}</p>
                    <p class="text-xs text-dark-400">{report.profile?.full_name || 'Unknown'}</p>
                  </div>
                  <button on:click={() => openReportResponse(report)} class="text-sm text-primary-400 hover:text-primary-300 px-2 py-1 rounded hover:bg-primary-900/20">
                    Respond
                  </button>
                </div>
              {/each}
  ...
  {:else if activeTab === 'bookings'}
    <div class="mb-4 flex gap-2 flex-wrap">
      {#each ['all', 'pending', 'approved', 'paid', 'confirmed', 'completed', 'cancelled'] as status}
        <button
          on:click={() => filterStatus = status}
          class="px-3 py-1.5 rounded-lg text-sm transition-colors
            {filterStatus === status ? 'bg-primary-600 text-white' : 'bg-dark-800 text-dark-400 hover:text-white'}"
        >
    ...
    {#if filteredBookings.length === 0}
      <div class="card text-center py-12"><p class="text-dark-400">No bookings found</p></div>
    {:else}
      <div class="space-y-3">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center gap-4">
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1">
                  <h3 class="font-semibold text-white">{booking.room?.name || 'Unknown Room'}</h3>
                  <span class={getStatusBadge(booking.status)}>{booking.status}</span>
                </div>
                <p class="text-sm text-dark-400">{booking.profile?.full_name || booking.guest_name} · {booking.guest_email}</p>
                <p class="text-sm text-dark-400">{formatDate(booking.date)} · {formatTime(booking.start_time)} - {formatTime(booking.end_time)}</p>
              </div>
              <div class="flex items-center gap-2 flex-wrap">
                {#if booking.status === 'pending'}
                  <button on:click={() => updateBookingStatus(booking.id, 'approved')} class="text-sm text-green-400 hover:text-green-300 px-3 py-1.5 rounded-lg hover:bg-green-900/20">Approve</button>
                {/if}
                {#if booking.status === 'approved'}
                  <button on:click={() => updateBookingStatus(booking.id, 'paid')} class="text-sm text-blue-400 hover:text-blue-300 px-3 py-1.5 rounded-lg hover:bg-blue-900/20">Mark Paid</button>
                {/if}
                {#if booking.status === 'paid'}
                  <button on:click={() => updateBookingStatus(booking.id, 'completed')} class="text-sm text-primary-400 hover:text-primary-300 px-3 py-1.5 rounded-lg hover:bg-primary-900/20">Complete</button>
                {/if}
                {#if booking.status !== 'cancelled'}
                  <button on:click={() => updateBookingStatus(booking.id, 'cancelled')} class="text-sm text-red-400 hover:text-red-300 px-3 py-1.5 rounded-lg hover:bg-red-900/20">Cancel</button>
                {/if}
              </div>
            </div>
          </div>
        ...
        <div class="card">
          <div class="flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-sm text-dark-400">{room.capacity} seats · {room.layout} · {formatCurrency(room.price_per_hour)}/hr</p>
            </div>
            <div class="flex items-center gap-2">
              <button
                on:click={() => toggleRoomActive(room.id, room.is_active)}
                class="text-sm {room.is_active ? 'text-yellow-400 hover:text-yellow-300' : 'text-green-400 hover:text-green-300'} px-3 py-1.5 rounded-lg hover:bg-dark-700 transition-colors"
              >
      ...
      {#each members as member (member.id)}
        <div class="card">
          <div class="flex flex-col sm:flex-row sm:items-center gap-4">
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1">
                <h3 class="font-semibold text-white">{member.full_name}</h3>
                <span class={member.is_approved ? 'badge-green' : 'badge-yellow'}>
                  {member.is_approved ? 'Approved' : 'Pending'}
                </span>
                <span class="badge bg-dark-700 text-dark-300 border-dark-600">{member.role}</span>
              </div>
              <p class="text-sm text-dark-400">{member.email} · {member.phone || 'No phone'}</p>
              <p class="text-xs text-dark-500">Joined {formatDate(member.created_at)}</p>
            </div>
            <div class="flex items-center gap-2">
              {#if !member.is_approved}
                <button on:click={() => approveMember(member.id)} class="text-sm text-green-400 hover:text-green-300 px-3 py-1.5 rounded-lg hover:bg-green-900/20">
                  Approve
                </button>
              {/if}
            </div>
  ...
  {:else if activeTab === 'reports'}
    {#if reports.length === 0}
      <div class="card text-center py-12"><p class="text-dark-400">No reports</p></div>
    {:else}
      <div class="space-y-3">
          ...
          <div class="card">
            <div class="flex flex-col sm:flex-row sm:items-start gap-4">
              <div class="flex-1 min-w-0">
                ...
                <p class="text-sm text-dark-400">From: {report.profile?.full_name || 'Unknown'} ({report.profile?.email || ''})</p>
                <p class="text-sm text-dark-300 mt-1">{report.description}</p>
              ...
              <div>
                <button on:click={() => openReportResponse(report)} class="text-sm text-primary-400 hover:text-primary-300 px-3 py-1.5 rounded-lg hover:bg-primary-900/20">
  ...
  {:else if activeTab === 'gallery'}
    ...
    {#if galleryImages.length === 0}
      <div class="card text-center py-12">
        <p class="text-dark-400 mb-4">No images uploaded yet</p>
...
<Modal isOpen={showImageUploadModal} title="Upload Image" on:close={() => showImageUploadModal = false}>
  <form on:submit|preventDefault={handleImageUpload} class="space-y-4">
    <div>
      <label class="block text-sm font-medium text-dark-200 mb-1">Image</label>
      <input
        type="file"
        accept="image/*"
    ...
    <div>
      <label class="block text-sm font-medium text-dark-200 mb-1">Title</label>
      <input type="text" bind:value={uploadTitle} class="input" placeholder="Image title" required />
    </div>

    <div>
      <label class="block text-sm font-medium text-dark-200 mb-1">Description <span class="text-dark-500">(optional)</span></label>
      <input type="text" bind:value={uploadDescription} class="input" placeholder="Brief description" />
    </div>

    <div>
      <label class="block text-sm font-medium text-dark-200 mb-1">Category</label>
      <select bind:value={uploadCategory} class="input">
  ...
  {#if reportToRespond}
    <form on:submit|preventDefault={submitReportResponse} class="space-y-4">
      <div class="bg-dark-800 rounded-lg p-3">
        <p class="text-sm text-white font-medium">{reportToRespond.subject}</p>
        <p class="text-sm text-dark-400 mt-1">{reportToRespond.description}</p>
      </div>

      <div>
        <label class="block text-sm font-medium text-dark-200 mb-1">Your Response</label>
        <textarea bind:value={adminResponse} class="input" rows="4" placeholder="Write your response..." required></textarea>
      </div>
