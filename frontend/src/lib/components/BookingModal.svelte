<script lang="ts">
	...
	let bookingCreatedTime: Date | null = null;

	// Multi-step wizard: 1 = plan & date, 2 = time & details, 3 = review & confirm.
	let step = 1;

	// Validity gates for moving forward
	...
	// fixed to 9 AM – 7 PM and the time selectors are hidden.
	$: fixedTimePlan =
		selectedPlan?.slug === 'full-day' || selectedPlan?.slug === 'weekly';

	// Auto-fill 9 AM – 7 PM for fixed-time plans once a date is picked.
	// Guard with lastFixedDate to prevent infinite reactive loops.
	let lastFixedDate = '';
	$: if (fixedTimePlan && selectedDate && selectedDate !== lastFixedDate) {
		lastFixedDate = selectedDate;
		startTime = '09:00';
		endTime = '19:00';
	}
	...
	// availability (mirrors the room page's server query).
	async function loadBookings() {
		...
		bookingsByDate = ((data ?? []) as Booking[]).reduce<Record<string, Booking[]>>(
			(acc, b) => {
				(acc[b.date] ??= []).push(b);
				return acc;
			},
	...
	// Auto-fill the guest's details from their signed-in profile.
	// Only start the timer once per modal open to avoid resetting it on every reactive update.
	$: if (isOpen) {
		...
		// Start the 15-minute pending payment timer only if not already running
	...
	// Business hours come from the shared availability engine
	...
	function startPendingTimer() {
		...
		pendingTimerInterval = setInterval(() => {
			...
			// Show warning at 5 minutes remaining (could add visual warning here)
	...
	// Select All button - sets time to 9:00 AM - 7:00 PM
	...
	{#if room}
		...
		>
			<div class="flex min-w-0 items-center gap-3">
				...
				/>
				<div class="min-w-0">
					<h3 class="truncate text-sm font-semibold text-dark-900">{room.name}</h3>
				...
				{#each stepTitles as title, i}
					...
					>
						<span class="flex items-center gap-1.5">
							...
							<span class={step === stepNum ? 'text-primary-700' : 'text-dark-500'}>
								{title}
							</span>
						</span>
					</button>
		...
		{#if step <= 3}
			...
			>
				<span class="inline-flex items-center gap-1.5">
					...
					Open {formatDisplayTime(minutesToTime(HUB_OPEN_HOUR * 60))} –
					{formatDisplayTime(minutesToTime(HUB_CLOSE_HOUR * 60))}
				</span>
				<span class="inline-flex items-center gap-1.5">
				...
				<div>
					...
					<div class="space-y-3">
						{#each plans as plan}
							...
							>
								<div class="min-w-0">
									<div class="font-semibold text-dark-900">{plan.name}</div>
									{#if plan.description}
										<p class="mt-0.5 text-xs text-dark-500">{plan.description}</p>
									{/if}
								</div>
								<div class="shrink-0 text-right">
									...
									<div class="mt-0.5 text-xs text-dark-500">{plan.duration_label}</div>
								</div>
							</button>
						{/each}
					</div>
				...
				<div>
					...
					{#if selectedDate}
						<div class="mt-4 rounded-xl border border-primary-100 bg-primary-50 p-4">
							...
							{:else}
								<p class="text-xs text-dark-500">No 1-hour blocks available on this day.</p>
							{/if}
				...
				<div>
					...
					{:else}
						...
						<div class="mb-3 flex justify-end">
							<button
								...
								on:click={selectAllTimeSlots}
							>
								Select All (9:00 AM – 7:00 PM)
							</button>
						</div>
						...
						<div class="grid grid-cols-1 gap-4">
							<div>
								...
								<div class="relative">
									...
									>
										<span>
											{startTime ? formatDisplayTime(startTime) : 'Select start time'}
										</span>
					...
					{#if !fixedTimePlan}
						<p class="mt-4 text-xs text-dark-500">
							...
							<span class="font-medium text-red-600">(Booked)</span> are already taken and
							can't be selected.
						</p>
					{/if}
				</div>
		...
		{#if step === 3}
					...
					{#if pendingMinutesRemaining > 0}
						<div class="mt-1 flex justify-between gap-4">
							...
							<span class="font-medium text-primary-600">
								{pendingMinutesRemaining} min
							</span>
						</div>
					{/if}
					{#if pendingMinutesRemaining === 10}
						<div class="mt-1 text-xs text-dark-500">
							Payment reminder: displayed at 10 minutes after booking creation, leaving ~5
			...
			<p class="mt-3 text-center text-xs text-dark-500">
				This reserves the {seriesDates.length > 1
					...
					: 'slot'} pending payment. Payment must be completed within 15 minutes or the
				booking will automatically expire and the room/time slot will be released.
			</p>
			...
			{#if selectedPlan?.slug !== 'full-day' && selectedPlan?.slug !== 'weekly'}
				<p class="mt-2 text-center text-xs text-dark-500">
					Payment reminder will be displayed at approximately 10 minutes after booking
					creation, leaving approximately 5 minutes before expiration.
				</p>
			{/if}
		{/if}
		...
		{#if step === 4 && showConfirmation}
			...
			<div class="mt-6">
				<button type="button" class="btn-primary w-full" on:click={close}>Done</button>
			</div>
		{/if}
	{/if}
</Modal>